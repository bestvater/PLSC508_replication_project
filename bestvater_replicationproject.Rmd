...
Larson & Lewis' thesis in their 2017 article is that information is disseminated less effectively in an ethnically-heterogeneous social network because even small differences in trust between groups will discourage inter-group communication and interrupt the flow. Empirically, this is demonstrated in a mached-village field experiment conducted in rural Uganda. Information about an upcoming event was seeded to a couple residents of each village, and then a few weeks later, researchers revisited the villages and took a census, asking residents to report their social ties, and also whether they had heard about/attended the event. Although the network in Mugana, the ethnically-heterogeneous village, was denser than the network in Abalang, the ethnically-homogeneous network, residents in Abalang were much more likely to have heard about the event. 

While these results are convincing, Larson & Lewis' analysis prompts an unadressed question: what about the ethnically-heterogeneous social network is preventing diffusion? What mechanisms are driving the disruption of information flows? Two scenarios seem plausible. In the first, villagers simply have a preference for social ties with co-ethnics. If these networks are characterized by preferential attachment by ethnicity, then we would expect the ethnically-heterogeneous village to be highly clustered, contributing to the interruption of information diffusion. This can be formally stated as:

**H1: Ethnically-heterogenous villages are characterized by preferential attachment by ethnicity.**

In the second scenario, villagers are equally likely to create social ties with other villagers of other ethnicities, but do not treat outgroup ties as equivalent to ties with coethnics. Put another way:

**H2: Coethnic ties are stronger than outgroup ties.**

A problem is presented here by the fact that the network data collected by Larson & Lewis are unweighted and do not allow H2 to be directly tested on observed data. However, we can get pretty far in adjudicating between the two scenarios by focusing on H1. If H1 is supported, we can take this as evidence that coethnic preferential attachment is driving the disruption of information diffusion in the ethnically-heterogeneous network. If H1 is not supported, however, it suggests that the network data as collected do not fully capture the nuances of social relationships in these villages, an idea which can then be further explored through simulation. In either case, prior to formal hypothesis testing it can be useful to explore the structure and dependencies of these networks descriptively to see if any structural patterns are suggestive of one explanation over the other.

## Comparing Centrality Distributions

We begin by comparing the degree distributions of each network, as shown below:
...
This figure demonstrates that both the Abalang and Mugana network appear to have relatively similar degree distributions. Both distributions are bimodal, with one peak around 10 incoming connections, and another around 0-1. Both networks appear to be characterized by a relatively well-connected core, with a smaller periphery of relatively isolated members. Notably, the Abalang network appears to have more isolates than the Mugana network, a fact that could be responsible for the marginally-higher density that Larson & Lewis note regarding the Mugana network. 
...
While the degree distributions of the Abalang and Mugana networks are similar, a comparison of their betweenness centrality distributions yields a dramatically different pattern. The Mugana network exhibits considerably higher levels of betweenness than the Abalang network does, suggesting that this network is more highly clustered, with fewer redundant paths between clusters. This is a pattern we might expect to observe if preferential attachment were occurring in the Mugana network.

## Comparing Graph-Level Characteristics
